...
error_code_t readTempLM75BD(uint8_t devAddr, float *temp) {
  ...
  RETURN_IF_ERROR_CODE(i2cSendTo(devAddr, writeBuff, TEMP_REG_WRITE_BUFF_SIZE));
  ...
  RETURN_IF_ERROR_CODE(i2cReceiveFrom(devAddr, readBuff, TEMP_READ_BUFF_SIZE));
  ...
  int16_t tempData = ((readBuff[0] << 8) | readBuff[1]) >> 5;
